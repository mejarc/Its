...
if (ie && !ie8) document.write("<iframe id='historyFrame' src='javascript:\"<html" + "></" + "html>\"' height='1' width='1' style='position:absolute;left:-800px'></iframe>");
...
if (ie) setTimeout(ieSlidyInit, 100);
else if (document.addEventListener) document.addEventListener("DOMContentLoaded", startup, false);
...
var helpText = "Navigate with mouse click, space bar, Cursor Left/Right, " + "or Pg Up and Pg Dn. Use S and B to change font size.";

var sizeIndex = 0;
var sizeAdjustment = 0;
var sizes = new Array("10pt", "12pt", "14pt", "16pt", "18pt", "20pt", "22pt", "24pt", "26pt", "28pt", "30pt", "32pt");
...
strings_es[helpText] = "Utilice el ratón, barra espaciadora, teclas Izda/Dcha, " + "o Re pág y Av pág. Use S y B para cambiar el tamaño de fuente.";
...
strings_ca[helpText] = "Utilitzi el ratolí, barra espaiadora, tecles Esq./Dta. " + "o Re pàg y Av pàg. Usi S i B per canviar grandària de font.";
...
strings_nl[helpText] = "Navigeer d.m.v. het muis, spatiebar, Links/Rechts toetsen, " + "of PgUp en PgDn. Gebruik S en B om de karaktergrootte te veranderen.";
...
strings_de[helpText] = "Benutzen Sie die Maus, Leerschlag, die Cursortasten links/rechts oder " + "Page up/Page Down zum Wechseln der Seiten und S und B für die Schriftgrösse.";
...
strings_pl[helpText] = "Zmieniaj slajdy klikając myszą, naciskając spację, strzałki lewo/prawo" + "lub PgUp / PgDn. Użyj klawiszy S i B, aby zmienić rozmiar czczionki.";
...
strings_fr[helpText] = "Naviguez avec la souris, la barre d'espace, les flèches " + "gauche/droite ou les touches Pg Up, Pg Dn. Utilisez " + "les touches S et B pour modifier la taille de la police.";
...
strings_hu[helpText] = "Az oldalak közti lépkedéshez kattintson az egérrel, vagy " + "használja a szóköz, a bal, vagy a jobb nyíl, illetve a Page Down, " + "Page Up billentyűket. Az S és a B billentyűkkel változtathatja " + "a szöveg méretét.";
...
strings_it[helpText] = "Navigare con mouse, barra spazio, frecce sinistra/destra o " + "PgUp e PgDn. Usare S e B per cambiare la dimensione dei caratteri.";
...
strings_el[helpText] = "Πλοηγηθείτε με το κλίκ του ποντικιού, το space, τα βέλη αριστερά/δεξιά, " + "ή Page Up και Page Down. Χρησιμοποιήστε τα πλήκτρα S και B για να αλλάξετε " + "το μέγεθος της γραμματοσειράς.";
...
strings_ja[helpText] = "マウス左クリック ・ スペース ・ 左右キー " + "または Page Up ・ Page Downで操作， S ・ Bでフォントサイズ変更";
...
strings_zh[helpText] = "用鼠标点击, 空格条, 左右箭头, Pg Up 和 Pg Dn 导航. " + "用 S, B 改变字体大小.";
...
strings_ru[helpText] = "Перемещайтесь кликая мышкой, используя клавишу пробел, стрелки" + "влево/вправо или Pg Up и Pg Dn. Клавиши S и B меняют размер шрифта.";
...
strings_sv[helpText] = "Bläddra med ett klick med vänstra musknappen, mellanslagstangenten, " + "vänster- och högerpiltangenterna eller tangenterna Pg Up, Pg Dn. " + "Använd tangenterna S och B för att ändra textens storlek.";
...
function startup() {
    ...
    if (!lang) lang = document.body.parentNode.getAttribute("xml:lang");
    ...
    if (opera) document.onkeypress = keyPress;
...
function resized() {
    ...
    else if (document.documentElement && document.documentElement.clientWidth) width = document.documentElement.clientWidth; // IE6
    ...
    else if (document.documentElement && document.documentElement.clientHeight) height = document.documentElement.clientHeight; // IE6
    ...
    if (width != lastWidth || height != lastHeight) {
        if (width >= 1100) sizeIndex = 5; // 4
        else if (width >= 1000) sizeIndex = 4; // 3
        else if (width >= 800) sizeIndex = 3; // 2
        else if (width >= 600) sizeIndex = 2; // 1
        else if (width) sizeIndex = 0;

        // add in font size adjustment from meta element e.g.
        // <meta name="font-size-adjustment" content="-2" />
        // useful when slides have too much content ;-)
        if (0 <= sizeIndex + sizeAdjustment && sizeIndex + sizeAdjustment < sizes.length) sizeIndex = sizeIndex + sizeAdjustment;
...
function getDocHeight(doc) // from document object
{
    ...
    if (doc && doc.body && doc.body.offsetHeight) return doc.body.offsetHeight; // ns/gecko syntax
    if (doc && doc.body && doc.body.scrollHeight) return doc.body.scrollHeight;

    alert("couldn't determine document height");
}

function getWindowHeight() {
    if (typeof(window.innerHeight) == 'number') return window.innerHeight; // Non IE browser
    if (document.documentElement && document.documentElement.clientHeight) return document.documentElement.clientHeight; // IE6
...
function keyDown(event) {
    ...
    if (isShownToc() && key != 9 && key != 16 && key != 38 && key != 40) {
        ...
        if (key == 27 || key == 84 || key == 67) return cancel(event);
    }
    ...
    {
        if (outline) {
            if (outline.visible) fold(outline);
            else
            unfold(outline);
...
function mouseButtonClick(e) {
    ...
    if (e.which) // all browsers except IE
    {
        ...
        if (e.button == 4) middleclick = true;
    ...
    if (mouseClickEnabled && leftclick && target.nodeName != "EMBED" && target.nodeName != "OBJECT" && target.nodeName != "VIDEO" && target.nodeName != "INPUT" && target.nodeName != "TEXTAREA" && target.nodeName != "SELECT" && target.nodeName != "OPTION") {
        nextSlide(true);
        stopPropagation(e);
        e.cancel = true;
        e.returnValue = false;
...
function nextSlide(incremental) {
    if (!viewAll) {
        ...
        if (incremental || slidenum == slides.length - 1) lastShown = revealNextItem(lastShown);
        ...
        if ((!incremental || lastShown == null) && slidenum < slides.length - 1) {
            ...
            if (last && incremental) lastShown = last;
        }
...
function setEosStatus(state) {
    if (eos) eos.style.color = (state ? "rgb(240,240,240)" : "red");
}
...
function showAllSlides() {
    ...
    for (var i = 0; i < slides.length; ++i) {
        ...
        try {
            if (i == 0) slide.style.pageBreakBefore = "avoid";
            else
...
function getClassList(element) {
    if (typeof element.className != 'undefined') return element.className;
...
function hasClass(element, name) {
    ...
    if (typeof element.className != 'undefined') return regexp.test(element.className);
...
function addClass(element, name) {
    if (!hasClass(element, name)) {
        if (typeof element.className != 'undefined') element.className += " " + name;
        else {
            var clsname = (ns_pos || ie8) ? "class" : "className";
...
function wrapImplicitSlides() {
    ...
    for (i = 0; i < headings.length; ++i) {
        ...
        while (node) {
            ...
            (node.nodeName == "H1" || node.nodeName == "h1" || node.nodeName == "DIV" || node.nodeName == "div")) break;
...
function collectSlides() {
    ...
    for (var i = 0; i < divs.length; ++i) {
        ...
        } else if (hasClass(div, "background")) { // work around for Firefox SVG reload bug
...
function matchingBackground(slideClass, bgClass) {
    if (bgClass == "background") return true;
    ...
    for (var i = 0; i < result.length; i++) {
        if (hasToken(bgClass, result[i])) return true;
    }
...
function nextIncrementalItem(node) {
    ...
    for (;;) {
        ...
        if (node.nodeType == 1) // ELEMENT
        {
            ...
            if (hasClass(node, "incremental") && okayForIncremental[node.nodeName]) return node;

            if (hasClass(node.parentNode, "incremental") && !hasClass(node, "non-incremental")) return node;
        }
    }
...
function clickedAnchor(e) {
    ...
    } else if (this.target == null) location.href = this.href;
...
function showSlideNumber() {
    slideNumElement.innerHTML = "slide".localize() + " " + (slidenum + 1) + "/" + slides.length;
}
...
function checkLocation() {
    ...
    if (slidenum > 0 && (hash == "" || hash == "#")) gotoSlide(0);
...
function setLocation() {
    ...
    if (slidenum >= 0) uri = uri + hash;

    if (ie && !ie8) pushHash(hash);

    if (uri != location.href /*&& !khtml */ ) location.href = uri;

    if (khtml) hash = "(" + (slidenum + 1) + ")";

    if (!ie && location.hash != hash && location.hash != "") location.hash = hash;
...
function pushHash(hash) {
    ...
    doc.write("<html><head><script type=\"text/javascript\">parent.onFrameLoaded('" + (hash) + "');</script></head><body>hello mum</body></html>");
    doc.close();
}
...
function findSlideNumber(uri) {
    ...
    for (i = 0; i < slides.length; ++i) {
        if (slides[i] == target) return i; // success
    }
...
function slideName(index) {
    ...
    if (heading) name = extractText(heading);

    if (!name) name = title + "(" + (index + 1) + ")";
...
function findHeading(node) {
    ...
    if (node.nodeName == "H1" || node.nodeName == "h1") return node;
...
function findCopyright() {
    ...
    for (var i = 0; i < meta.length; ++i) {
        ...
        if (name == "copyright") return content;
    }
...
function findSizeAdjust() {
    ...
    for (var i = 0; i < meta.length; ++i) {
        ...
        if (name == "font-size-adjustment") return 1 * content;
    }
...
function isShownToc() {
    if (toc && toc.style.visible == "visible") return true;
...
function showTableOfContents() {
    if (toc) {
        if (toc.style.visibility != "visible") {
            ...
            if (ie7 && slidenum == 0) setTimeout("ieHack()", 100);
        } else
        hideTableOfContents();
    }
}
...
function toggleTableOfContents() {
    if (toc) {
        if (toc.style.visible != "visible") showTableOfContents();
        else
        hideTableOfContents();
    }
}
...
function tableOfContents() {
    ...
    for (var i = 0; i < slides.length; ++i) {
        ...
        if (title) addClass(a, "titleslide");
    ...
    toc.onclick = function (e) {
        ...
        if (selectedTextLen <= 0) hideTableOfContents();
...
function initOutliner() {
    ...
    for (var i = 0; i < items.length; ++i) {
        ...
        if (!hasClass(target.parentNode, "outline")) continue;
        ...
        if (foldable(target)) {
            ...
            if (hasClass(target, "expand")) unfold(target);
            else
            fold(target);
        } else {
...
function foldable(item) {
    ...
    while (node) {
        if (node.nodeType == 1 && isBlock(node)) return true;
...
function hoverOutline(e) {
    ...
    if (target && target.foldable) target.style.cursor = "pointer";
...
function isBlock(elem) {
    ...
    return tag == "OL" || tag == "UL" || tag == "P" || tag == "LI" || tag == "TABLE" || tag == "PRE" || tag == "H1" || tag == "H2" || tag == "H3" || tag == "H4" || tag == "H5" || tag == "H6" || tag == "BLOCKQUOTE" || tag == "ADDRESS";
}
